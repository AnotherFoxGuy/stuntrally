...
vdrift::optional <const BEZIER *> ROADSTRIP::FindBezierAtOffset(const BEZIER * bezier, int offset) const
{
	...
	if (it == patches.end())
		return vdrift::optional <const BEZIER *>(); //return nothing
	else
	{
		//now do the offset
		...
		return vdrift::optional <const BEZIER *>(&it->GetPatch());
	}
}
